...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        #randomly choosing the index from the dataset
        temp=np.random.choice(len(XX), n, replace=True)
        
        #identifying  samples based on the random index.
        [samples.append(XX[x][0:-1]) for x in temp]
        
        #identifying  labels based on the random index.
        [labels.append(XX[x][-1]) for x in temp]
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                for i in range(len(self.bootstraps_datasets)):
                    ...
                    notall_OOB_tree = self.decision_trees[i]
                    #classifying the record on all the trees as this record is present on all
                    effective_vote = notall_OOB_tree.classify(record)
                    votes_not_bagged_all.append(effective_vote)
                
                counts_notall = np.bincount(votes_not_bagged_all)
                #majority voting
                y = np.append(y, np.argmax(counts_notall))    
